...
const useStyles = makeStyles((theme) => ({
  ...
  const handleReopenTicket = async (id) => {
    ...
    const handleAcepptTicket = async (id) => {
        ...
        const msg = `{{ms}} *{{name}}*, mi nombre es *${user?.name}* y ahora seguiré con tu servicio!`;
        const message = {
            read: 1,
            fromMe: true,
            mediaUrl: "",
  ...
  return (
    <React.Fragment key={ticket.id}>
      ...
      >
        <Tooltip arrow placement="right" title={ticket.queue?.name?.toUpperCase() || "SIN AREA"} >
            ...
            <span className={classes.contactNameWrapper}>
              ...
              > {ticket.lastMessage.includes('data:image/png;base64') ? <MarkdownWrapper> Ubicación</MarkdownWrapper> : <MarkdownWrapper>{ticket.lastMessage}</MarkdownWrapper>}
                <span className={classes.secondaryContentSecond} >
                  ...
                  <Badge style={{ backgroundColor: ticket.queue?.color || "#7c7c7c" }} className={classes.connectionTag}>{ticket.queue?.name?.toUpperCase() || "SIN AREA"}</Badge>
                </span>
